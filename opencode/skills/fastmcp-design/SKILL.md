---
name: fastmcp-design
description: Design FastMCP servers with tools, resources, and prompts using MCP Framework v6.0.0. Use when planning MCP servers, designing tool decorators, or architecting Model Context Protocol integrations. Orchestrates steps 1, 3, 9-11 (design intent).
allowed-tools: [*]
metadata:
...
# FastMCP Design Orchestrator

**Framework:** MCP v6.0.0 | **Intent:** design | **Tech:** fastmcp

## Auto-Activation

I activate when you mention:
- "design FastMCP [server/tool/resource]"
- "plan MCP [integration/server]"
- "architect FastMCP [application/service]"
- "MCP tool design/structure"
...
**Examples:**
> "Design a FASTMCP authentication service with auto-approval"
> "Design a FASTMCP user management, auto-approve all gates" 
...
**ALL FastMCP design work MUST follow the MCP workflow - ZERO EXCEPTIONS:**
...
1. **CONTEXT LOAD**: Use `neo4j-memory` to load previous MCP server architectural decisions
2. **🔴 MANDATORY RESEARCH**: Use `context7` + `grep` for FastMCP/MCP patterns BEFORE designing
   - context7: Latest FastMCP/MCP protocol specifications, tool design patterns
   - grep: Real-world FastMCP servers and MCP tool implementations from GitHub
3. **PLANNING**: Use `sequential-thinking` to structure MCP server design
4. **DESIGN**: Create FastMCP server architecture/tool design
5. **PROGRESS TRACKING**: Record MCP design decisions to `neo4j-memory`
6. **CONTEXT SAVE**: Persist FastMCP patterns and decisions to `neo4j-memory`

**WHY THIS IS CRITICAL FOR FASTMCP DESIGN:**
- context7 ensures you use CURRENT MCP protocol specifications
- grep shows you REAL production FastMCP server implementations
- neo4j-memory preserves MCP design decisions and tool patterns

**See:** `.claude/skills/mcp-tools-workflow/SKILL.md` for complete workflow

**ABSOLUTELY FORBIDDEN:**
- ❌ Designing FastMCP without context7 + grep research
- ❌ Skipping neo4j-memory context load
- ❌ Completing design without saving MCP decisions to neo4j-memory

## Execution

**READ**: `.mcp_prompts/platforms/fastmcp/run.fastmcp-design.prompt.yaml`
...
- Tool decorator specifications
- Resource handler design
...
`fastmcp-planning`, `fastmcp-implement`, `python-design`
